...
namespace Machine.Specifications.ConsoleRunner.Specs
{
    ...
    : ConsoleRunnerSpecs
  {
    ...
    It should_output_an_error_message_with_the_name_of_the_missing_assembly = ()=>
      console.Lines.Should().Contain(string.Format("Missing Assembly: {0}", missingAssemblyName));
  ...
  {
    Because of = () =>
      program.Run(new[] { GetPath(@"Example.UsingExternalFile.dll"), "--no-appveyor-autodetect" });
